...
export type FacetKey = "artist" | "member" | "season" | "class" | "collectionNo";

export type FacetValues = Record<FacetKey, string[]>;

export const EMPTY_FACET_VALUES: FacetValues = {
  artist: [],
  member: [],
  season: [],
  class: [],
...
export const FACETS: readonly FacetDef[] = [
  { key: "artist", label: "Artist", options: (f) => f.artists, Control: MultiSelect },
  {
    key: "member",
    label: "Member",
    options: (f) => f.members,
  ...
  { key: "season", label: "Season", options: (f) => f.seasons, Control: MultiSelect },
  { key: "class", label: "Class", options: (f) => f.classes, Control: MultiSelect },
  {
    key: "collectionNo",
    label: "Collection",
 ...
 * both surfaces — the profile Activity tab's Event select. It goes through the same
 * parity guard as `FACETS`, so it cannot quietly go missing from the sheet,
 * and it declares whether it holds a non-default value so the sheet trigger's
 * badge can count it.
 */
...
/**
 * How a control is dressed for its surface: bare in the inline toolbar row,
 * labelled and full-width in the mobile sheet. Both the facet table and the
 * extras above go through this, so the two kinds of control cannot drift
 * apart inside the sheet.
 */
...
/** Renders the non-facet controls for one surface. */
...
/** Renders the facet controls for one surface. */
export function FacetControls({
  ...
  return FACETS.filter((def) => keys.includes(def.key)).map(
    ({ key, label, options, grouped, Control }) => {
      ...
      return forSurface(surface, key, label, control);
    },
  );
}

/**
 * Dev-only guard against the bug this array exists to prevent: a facet that
 ...
 * "every control on one surface is on the other", and an extra like the Type
 * select is exactly the sort of control that would otherwise go missing.
 */
...
export function useFacetParity(): void {
  ...
  useEffect(() => {
    ...
    const missing = inline.filter((k) => !stacked.includes(k));
    const extra = stacked.filter((k) => !inline.includes(k));
