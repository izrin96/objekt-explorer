...
RUN --mount=type=secret,id=umami_script_url \
    ...
    VITE_UMAMI_SCRIPT_URL=$(cat /run/secrets/umami_script_url) \
    VITE_UMAMI_WEBSITE_ID=$(cat /run/secrets/umami_website_id) \
    VITE_ACTIVITY_WEBSOCKET_URL=$(cat /run/secrets/activity_websocket_url) \
    ...
    VITE_SITE_URL=$(cat /run/secrets/site_url) \
    ...
    VITE_LIVE_API_KEY=$(cat /run/secrets/live_api_key) \
