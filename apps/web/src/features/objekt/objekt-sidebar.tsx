...
// so users with a non-16px root got oversized glyphs at fixed anchors — text
// drifted off the band.
...
export function ObjektSidebar({ objekt, hideSerial = false }: Props) {
  return (
    /* hidden from the a11y tree: every value on the band is already text next
       to the card, and the band image's alt lands in the card's name */
    <div
      aria-hidden="true"
      className="pointer-events-none absolute inset-0 isolate grid items-center select-none [&>*]:col-start-1 [&>*]:row-start-1"
