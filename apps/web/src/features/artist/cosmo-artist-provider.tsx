...
export function CosmoArtistProvider({ children }: PropsWithChildren) {
    ...
    () =>
      new Map(
        artists.flatMap((artist) =>
          artist.artistMembers.map((member) => [member.name.toLowerCase(), member] as const),
        ),
      ),
    [artists],
  );
...
export function useCosmoArtist() {
  const ctx = use(CosmoArtistContext);
  ...
  // an empty cookie means "every artist", which is what the server scopes to
  ...
  const compareMember = useCallback(
    (memberA: string, memberB: string) => {
      const orderA = ctx.memberMap.get(memberA.toLowerCase())?.order ?? -1;
      const orderB = ctx.memberMap.get(memberB.toLowerCase())?.order ?? -1;
