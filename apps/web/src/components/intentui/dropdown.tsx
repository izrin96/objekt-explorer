...
const dropdownSectionStyles = tv({
  slots: {
    ...
    header: "text-muted-fg col-span-full px-2.5 py-1.5 text-xs/3 font-medium",
  },
});
...
const dropdownItemStyles = tv({
  base: [
    "min-w-0 [--me-icon:--spacing(2)]",
    "col-span-full grid grid-cols-[auto_1fr_1.5rem_0.5rem_auto] px-2.5 py-1.5 supports-[grid-template-columns:subgrid]:grid-cols-subgrid",
    ...
    // text
    "text-fg text-sm/6 forced-colors:text-[CanvasText]",
    // avatar
    "*:data-[slot=avatar]:*:me-(--me-icon) *:data-[slot=avatar]:me-(--me-icon) *:data-[slot=avatar]:[--avatar-size:--spacing(4)] has-[[slot=description]]:*:data-[slot=avatar]:row-span-2",
    // icon
    ...
    "not-has-[[slot=description]]:*:[svg]:size-4",
    "has-[[slot=description]]:[&_svg:not([class*='w-'])]:w-4 has-[[slot=description]]:*:[svg]:h-lh",
...
const DropdownKeyboard = ({ className, ...props }: React.ComponentProps<typeof Keyboard>) => {
  return (
    <Keyboard
      className={twMerge(
        "group-hover:text-primary-fg group-focus:text-primary-fg absolute end-2 ps-2",
        className,
      )}
      {...props}
    />
