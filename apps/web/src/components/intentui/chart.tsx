    ...
    <ChartContext value={value}>
      <div
        ...
        className={twMerge(
          ...
          "[&_.recharts-cartesian-axis-tick_text]:fill-muted-fg [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-border/80 [&_.recharts-curve.recharts-tooltip-cursor]:stroke-border [&_.recharts-polar-grid_[stroke='#ccc']]:stroke-border [&_.recharts-radial-bar-background-sector]:fill-muted [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-muted [&_.recharts-reference-line_[stroke='#ccc']]:stroke-border [&_.recharts-surface_.recharts-text.recharts-cartesian-axis-tick-value]:*:fill-muted-fg [&_.recharts-layer]:outline-hidden [&_.recharts-sector]:outline-hidden [&_.recharts-sector[stroke='#fff']]:stroke-transparent [&_.recharts-surface]:outline-hidden",
...
const ChartTooltipContent = <TValue extends ValueType, TName extends NameType>({
  ...
  return (
    <div
      ref={ref}
      className={twMerge(
        "bg-overlay/70 text-overlay-fg grid min-w-48 items-start rounded-lg p-3 py-2 text-xs ring ring-current/10 backdrop-blur-lg",
        className,
      )}
    >
      {!hideLabel && (
        <>
          ...
          {labelSeparator && <span aria-hidden className="bg-bg/10 mt-2 mb-3 block h-px w-full" />}
        </>
      )}
        ...
        {payload.map((item, index) => {
          ...
          return (
            <div
              key={key}
              className={twMerge(
                "*:[svg]:text-muted-fg flex w-full flex-wrap items-stretch gap-2",
      ...
      {payload.map((item: LegendPayload) => {
        ...
        return (
          <ToggleButton
            ...
            id={key}
            className={twMerge(
              "text-muted-fg *:[svg]:text-muted-fg flex items-center gap-2 rounded-sm px-2 py-1 *:[svg]:-mx-0.5 *:[svg]:size-2.5 *:[svg]:shrink-0",
