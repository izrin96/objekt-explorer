...
import { DEFAULT_ADMIN_ROLE, ERROR_ALL_VOTES_REVEALED, ERROR_EMPTY_CALLDATA, ERROR_INSUFFICIENT_TOKEN_AMOUNT, ERROR_INVALID_CANDIDATES_LENGTH, ERROR_INVALID_DATA_LENGTH, ERROR_INVALID_DUE, ERROR_INVALID_FROM_ADDRESS, ERROR_INVALID_MINIMUM_TOKEN, ERROR_INVALID_OFFSET, ERROR_INVALID_REVEAL_DATA, ERROR_INVALID_START_AT, ERROR_INVALID_TOKEN_UNIT, ERROR_INVALID_VOTE_SIGNER_ADDRESS, ERROR_INVALID_VOTE_UNIT, ERROR_NOT_ALL_VOTES_REVEALED, ERROR_NOT_IN_PROGRESS, ERROR_POLL_ALREADY_EXISTS, ERROR_POLL_ALREADY_FINALIZED, ERROR_POLL_NOT_EXISTS, ERROR_VOTE_HASH_CANNOT_BE_REPLAYED, ERROR_VOTE_SIGNER_INVALID_SIGNATURE, OPERATOR_ROLE, UPGRADE_INTERFACE_VERSION, candidates, eip712Domain, getRoleAdmin, hasRole, onERC1155BatchReceived, onERC1155Received, proxiableUUID, remainingVotesCount, supportsInterface, totalVotesCount, voteSignerAddress, votesPerCandidates } from './functions.js'
import type { CandidatesParams, GetRoleAdminParams, HasRoleParams, OnERC1155BatchReceivedParams, OnERC1155ReceivedParams, RemainingVotesCountParams, SupportsInterfaceParams, TotalVotesCountParams, VotesPerCandidatesParams } from './functions.js'
...
export class Contract extends ContractBase {
    ...
    onERC1155BatchReceived(operator: OnERC1155BatchReceivedParams["operator"], from: OnERC1155BatchReceivedParams["from"], tokenIds: OnERC1155BatchReceivedParams["tokenIds"], values: OnERC1155BatchReceivedParams["values"], data: OnERC1155BatchReceivedParams["data"]) {
        return this.eth_call(onERC1155BatchReceived, {operator, from, tokenIds, values, data})
    }

    onERC1155Received(operator: OnERC1155ReceivedParams["operator"], from: OnERC1155ReceivedParams["from"], tokenId: OnERC1155ReceivedParams["tokenId"], amount: OnERC1155ReceivedParams["amount"], data: OnERC1155ReceivedParams["data"]) {
    ...
    remainingVotesCount(tokenId: RemainingVotesCountParams["tokenId"], pollId: RemainingVotesCountParams["pollId"]) {
    ...
    totalVotesCount(tokenId: TotalVotesCountParams["tokenId"], pollId: TotalVotesCountParams["pollId"]) {
    ...
    votesPerCandidates(tokenId: VotesPerCandidatesParams["tokenId"], pollId: VotesPerCandidatesParams["pollId"]) {
