...
The Pinned shelf is gone by the user's decision (FIX11): pins lead the grid as `pinFirst` already ordered them, and the owner reorders them in place — the drag context now wraps the virtual grid and each pinned cell is the sortable.
