...
export async function exchangeLoginTicket(recaptchaToken: string) {
  return await cosmoShop<AuthTicket>(`/bff/v1/users/auth/login/native/qr/ticket`, {
    method: "POST",
    body: {
      recaptcha: {
        action: "login",
...
export async function queryTicket(ticket: string) {
  return await cosmoShop<QueryTicket>(`/bff/v1/users/auth/login/native/qr/ticket`, {
    query: {
      tid: crypto.randomUUID(),
      ticket,
    },
...
export async function certifyTicket(otp: number, ticket: string) {
  return await cosmoShop.raw<void>(`/bff/v1/users/auth/login/native/qr/ticket/certify`, {
    method: "POST",
    body: {
      otp,
      ticket,
